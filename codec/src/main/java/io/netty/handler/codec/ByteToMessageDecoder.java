...
public abstract class ByteToMessageDecoder extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof ByteBuf) {
            ...
            try {
                ...
                // 解码消息的数量
                int size = out.size();
                // 是否解码到消息。为什么不直接使用 size 来判断呢？因为如果添加了消息，然后又移除该消息，
                // 此时 size 为 0 ，但是 !out.insertSinceRecycled() 为 true
            ...
            //消息的类型不是 ByteBuf 类，直接触发 Channel Read 事件到下一个节点。也就说，不进行解码
    ...
    protected void callDecode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
        try {
            ...
            while (in.isReadable()) {
                ...
                if (outSize > 0) {// out 非空，说明上一次解码有解码到消息
                    // 触发 Channel Read 事件。可能是多条消息
     ...
     * <p>
     * 也是因为 Cumulator 的累加，所以能将不完整的包累加到一起，从而完整。当然，累加的过程，没准又进入了一个不完整的包。
     * 所以，这是一个不断累加，不断解码拆包的过程。
     */
