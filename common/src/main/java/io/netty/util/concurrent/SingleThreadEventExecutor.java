...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
     ...
     * threadLock是一个初始值为0的信号量。一个初值为0的信号量，当线程请求锁时只会阻塞，
     */
    ...
    protected static Runnable pollTaskFrom(Queue<Runnable> taskQueue) {
        for (; ; ) {
            ...
            //调用 Queue#poll() 方法，获得并移除队首元素。如果获得不到，返回 null 。注意，这个操作是非阻塞的
    ...
    protected boolean runAllTasks() {
        ...
        do {
            ...
            //调用 #runAllTasksFrom(taskQueue) 方法，执行任务队列中的所有任务
    ...
    protected boolean runAllTasks(long timeoutNanos) {
        ...
        if (task == null) {
            //未取到任务，执行所有任务完成的后续方法，就是讲任务队列中的任务全部执行
            afterRunningAllTasks();
            return false;
        }
        //计算执行任务截止时间
        ...
        for (; ; ) {
            ...
            //每隔 64 个任务检查一次时间，因为 System#nanoTime() 是相对费时的操作。也因此，超过执行时间上限是“近似的”，而不是绝对准确。
            if ((runTasks & 0x3F) == 0) {
                lastExecutionTime = ScheduledFutureTask.nanoTime();
                //超时则结束
     ...
     * @return 其中的参数quietPeriod为静默时间，timeout为截止时间，此外还有一个相关参数gracefulShutdownStartTime即优雅关闭开始时间
     */
    @Override
    ...
    protected boolean confirmShutdown() {
        ...
        if (runAllTasks() || runShutdownHooks()) {
            ...
            //此处调用wakeup()唤醒线程执行任务，因为return false 所以run()线程不会退出循环 将继续执行
            return false;
        }
